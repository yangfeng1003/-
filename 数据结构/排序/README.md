![在这里插入图片描述](https://img-blog.csdnimg.cn/20190620093445996.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MzMyMjIwOA==,size_16,color_FFFFFF,t_70)

![在这里插入图片描述](https://img-blog.csdnimg.cn/20190620093904174.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MzMyMjIwOA==,size_16,color_FFFFFF,t_70)
...
### 4.希尔排序（缩小增量排序）
希尔排序是按一定的间隔对数列进行分组，然后在每一个分组中做插入排序；随后逐次缩小间隔。它与插入排序的不同之处在于，它会优先比较距离较远的元素
```js
...
快速排序法为什么一定要从右边开始：  
以6 1 2 7 9为例，交换后的数组为：7 1 2 6 9 是有问题的。因为当我们先从在边开始时，i所停留的那个位置肯定是大于基数6的
```js
function quickSort1(arr){ //方法1，辅助数组，很简单
...
function quickSort2(arr,left,right){  // 2变种  left right指针互换
...
function  quickSort3(array,left,right) {  //方法3  选取最后一个元素为枢轴，i是枢轴正确放置的位置（效率并不高）
...
// let arr = [2,6,4,9,12,3,20,8,1,3,44,38,5,47,15,36,26,27,2,46,4,19,50,48,3, 44, 38, 5, 47, 15, 36, 26, 27, 2, 46, 4, 19, 50, 48, 3, 44, 38, 5, 47, 15, 36, 26, 27, 2, 46, 4, 19, 50, 48];
console.time("冒泡排序");
