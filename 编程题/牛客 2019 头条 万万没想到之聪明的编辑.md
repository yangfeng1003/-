...
/*public class DelectKToMaxValue {
    ...
    public static String[] check(String[] arr) {
    	for(int i=0;i<arr.length;i++) {
    		...
    		while(j<str.length()-3) {
    			//检测三个相同字符
    			char a = str.charAt(j);
    			if(str.charAt(j+1) == a && str.charAt(j+2) == str.charAt(j+1)) {
    				str.delete(j+1,j+2);
    			}
    			//检测aabb
    			else if(str.charAt(j+1) == a && str.charAt(j+1) != str.charAt(j+2) && str.charAt(j+2) == str.charAt(j+3)) {
    				str.delete(j+2,j+3);
    			}else {
    				j++;
    			}
